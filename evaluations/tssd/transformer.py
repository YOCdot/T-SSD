...
class VisionTransformer(nn.Module):
    ...
    def return_all_attn(self, x):
        ...
        return {
            "before": out_list,
            "after": [token_1, token_2, token_3, token_4]  # x
        }

    def forward(self, x):
        if self.return_attn:
            return self.return_all_attn(x)
        else:
            return self.forward_features(x)


def deit_tiny_patch16_224(pretrained=None, return_attn=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), name="tiny",
        return_attn=return_attn, **kwargs)
...
def deit_small_patch16_224(pretrained=None, return_attn=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), name="small",
        return_attn=return_attn, **kwargs)
    # kernel_list = [4, 6, 6, 7, 8]
    # stride_list = [1, 1, 1, 1, 1]
    # head_list = [3, 4, 4, 6, 6]
...
def deit_base_patch16_224(pretrained=None, return_attn=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), name="base",
        return_attn=return_attn, **kwargs)
    # kernel_list = [4, 6, 6, 7, 8]
    # stride_list = [1, 1, 1, 1, 1]
    # head_list = [3, 4, 6, 6, 6]
