...
from .transformer import deit_tiny_patch16_224, deit_small_patch16_224, deit_base_patch16_224
from .encoder import encoder_tiny_672_192, encoder_small_672_384, encoder_base_768_768
...
class Detector(nn.Module):
    ...
    def forward_features(self, samples: NestedTensor):
    ...
    def return_all_attn(self, samples: NestedTensor):
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        return self.backbone(samples.tensors)

    def forward(self, samples: NestedTensor):
        if self.return_attn:
            return self.return_all_attn(samples)
        else:
            return self.forward_features(samples)
...
def swin_ssd_tiny(encoder_pt=None, deit_pt=None, det_token_num=100, neck_pt=None, num_classes=91, return_attn=False):
    encoder = encoder_tiny_672_192(pretrain=encoder_pt)
    neck = deit_tiny_patch16_224(pretrained=deit_pt, return_attn=return_attn)
    ...
    print("SwinSSD-Tiny Successfully Built")

    return detector


def swin_ssd_small(encoder_pt=None, deit_pt=None, det_token_num=100, neck_pt=None, num_classes=91, return_attn=False):
    encoder = encoder_small_672_384(pretrain=encoder_pt)
    neck = deit_small_patch16_224(pretrained=deit_pt, return_attn=return_attn)
    detector = Detector(encoder=encoder, backbone=neck,
                        token_len=sum(encoder.num_list),
    ...
    print("SwinSSD-Small Successfully Built")

    return detector


def swin_ssd_base(encoder_pt=None, deit_pt=None, det_token_num=100, neck_pt=None, num_classes=91, return_attn=False):
    encoder = encoder_base_768_768(pretrain=encoder_pt)
    neck = deit_base_patch16_224(pretrained=deit_pt, return_attn=return_attn)
    detector = Detector(encoder=encoder, backbone=neck,
                        token_len=sum(encoder.num_list),
...
def build(args):
    ...
    if args.model_name == "tiny":
        model = swin_ssd_tiny(encoder_pt=args.encoder_pt, deit_pt=None, det_token_num=args.det_token_num,
                              neck_pt=args.neck_pt, num_classes=num_classes,
                              return_attn=args.return_attn)
    elif args.model_name == 'small':
        model = swin_ssd_small(encoder_pt=args.encoder_pt, deit_pt=None, det_token_num=args.det_token_num,
                               neck_pt=args.neck_pt, num_classes=num_classes,
                               return_attn=args.return_attn)
    elif args.model_name == "base":
        model = swin_ssd_base(encoder_pt=args.encoder_pt, deit_pt=None, det_token_num=args.det_token_num,
                              neck_pt=args.neck_pt, num_classes=num_classes,
                              return_attn=args.return_attn)
    else:
...
def build_tssd(model_name="tiny"):
    ...
    num_classes = 91
    args = Params(model_name=model_name)
...
if __name__ == "__main__":
    ...
    # 测试
    img = cv2.imread('/home/yoc/codes/train_image.jpg')
    # INPUT-672
    ...
    tssd = swin_ssd_small().to(device)
    tssd.train()
    macs, params = thop.profile(tssd, inputs=(input672,))
